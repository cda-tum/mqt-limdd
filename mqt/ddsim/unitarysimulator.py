...
class UnitarySimulator(BackendV1):
    ...
    def run_experiment(self, qobj_experiment: QasmQobjExperiment, **options):
        ...
        if mode == "sequential":
            ...
            msg = "Construction mode"
            raise DDSIMError(
                msg, mode, "not supported by DDSIM unitary simulator. Available modes are 'recursive' and 'sequential'"
            )
    ...
    def _validate(self, qobj):
        ...
        if n_qubits > max_qubits:
            raise DDSIMError(
                f"Number of qubits {n_qubits} " + f"is greater than maximum ({max_qubits}) " + f'for "{self.name()}".'
            )
        ...
        for experiment in qobj.experiments:
            ...
            for operation in experiment.instructions:
                if operation.name in ["measure", "reset"]:
                    raise DDSIMError(
                        'Unsupported "%s" instruction "%s" ' + 'in circuit "%s" ', self.name(), operation.name, name
                    )
