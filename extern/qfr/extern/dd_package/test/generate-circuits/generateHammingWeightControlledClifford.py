...
def writePrologue(file_out, n):
    file_out.write(
        "// This circuit is a randomly generated Hamming Weight-controlled circuit. The control and target registers have "
        + str(n)
        + " qubits each\n\n"
    )
...
def writeHammingControlComputePart(file_out, n, k):
    ...
    for w in range(0, k + 1):
        ...
        file_out.write(
            ...
            + getIndex(n, k)
            + "], q["
            + getAncillaRegisterIndex(n, w)
            + "], q["
            + getTargetRegisterIndex(n, w)
            + "];\n"
        )
        file_out.write("x q[" + getIndex(n, k) + "];\n")
        file_out.write(
...
def writeComputePartStage1(file_out, n):
    ...
    file_out.write(
        "ccx q[" + getIndex(n, 0) + "], q[" + getIndex(n, 1) + "], q[" + getAncillaRegisterIndex(n, 0) + "];\n"
    )
    file_out.write("x q[" + getIndex(n, 0) + "];\n")
    file_out.write("x q[" + getIndex(n, 1) + "];\n")
    # not 0, 1, then 1
    file_out.write("x q[" + getIndex(n, 0) + "];\n")
    file_out.write(
        "ccx q[" + getIndex(n, 0) + "], q[" + getIndex(n, 1) + "], q[" + getAncillaRegisterIndex(n, 1) + "];\n"
    )
    file_out.write("x q[" + getIndex(n, 0) + "];\n")
    # 0, not 1, then 1
    file_out.write("x q[" + getIndex(n, 1) + "];\n")
    ...
    # 0 and 1 then 2
    file_out.write(
        "ccx q[" + getIndex(n, 0) + "], q[" + getIndex(n, 1) + "], q[" + getAncillaRegisterIndex(n, 2) + "];\n"
    )
...
def writeRandomCliffordCircuit(file_out, n, nGates):
    file_out.write(
        ...
        + " Hadamard gates and "
        + str(nGates - n)
        + " random controlled Clifford gates\n"
    )
    ...
    for _g in range(0, nGates - n):
        ...
        if random.randint(0, 1) == 0:
            ...
            file_out.write(
                ...
                + getAncillaRegisterIndex(n, w)
                + "],q["
                + getTargetRegisterIndex(n, target)
                + "];\n"
            )
        else:
            ...
            file_out.write(
                ...
                + getAncillaRegisterIndex(n, w)
                + "],q["
                + getTargetRegisterIndex(n, control)
                + "],q["
                + getTargetRegisterIndex(n, target)
                + "];\n"
            )
...
if len(sys.argv) < 2:
    print(
        "Usage: "
        + sys.argv[0]
        + " nQubits [nGates], where nQubits is the number of qubits, nGates is the number of controlled Clifford gates.\nBy default nGates = 4* nQubits ^ 2"
    )
else:
